...
from common.file_handler import path_create, filename_duplicates, read_yaml
from common.logger import Logger


# 可以将基类下面的函数定义为私有函数，更规范（函数名前加两个下划线）
class BasePage:
    datas = read_yaml('data/bysms.yaml')
    base_url = datas['url']
    ...
    def _open(self, url):
        self.log.info(f'打开url: {self.base_url + url}')
    ...
    def _switch_to_iframe(self, frame):
        try:
            self.log.info(f'切换到iframe')
            # 传入的frame参数可能是下标、name属性、webElement元素
    ...
    def _select_option_by_text(self, select, txt):
        ...
        try:
            ...
            self.log.error(f'下拉框的{txt}选项选择失败。错误信息：{e}')
            return False
    ...
    def _select_option_by_index(self, select, index):
        ...
        try:
            ...
            self.log.error(f'下拉框的第{index}个选项选择失败。错误信息：{e}')
            return False
    ...
    def _select_option_by_value(self, select, value):
        ...
        try:
            ...
            self.log.error(f'选择下拉框中value为{value}的选项失败。错误信息：{e}')
            return False
